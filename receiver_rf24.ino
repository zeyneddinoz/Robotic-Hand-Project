...
void loop(){
  if(radio.available()){
    ...
    if(msg[0] <11 && msg[0] >-1){
      data = msg[1], pos=map(data, 0, 10, 90, 0);
      myServo1.write(pos); 
    }
    if(msg[0] <21 && msg[0]>10){
      data = msg[1], pos=map(data, 11, 20, 90, 0);
      myServo3.write(pos);
    }
    if(msg[0] <31 && msg[0]>20){
      data = msg[0], pos=map(data, 21, 30, 90, 0);
      myServo4.write(pos);
    }
    if(msg[0] <41 && msg[0]>30){
      data = msg[0], pos=map(data, 31, 40, 90, 0);
      myServo2.write(pos);
    }
    if(msg[0] <51 && msg[0]>40){
      data = msg[0], pos=map(data, 41, 50, 90, 0);
      myServo5.write(pos);
    }
  }
}
